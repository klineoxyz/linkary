/**
 * GET /api/me/discovery/orgs
 *
 * Paid discovery: returns discovery-safe org list for eligible authenticated users only.
 * Contract: only allowlisted fields. No internal ids or sensitive data.
...
import { getDiscoveryOrgs } from "@/lib/discoveryService";
import { validateDiscoveryQuery } from "@/lib/discoveryValidation";
import { shapeDiscoveryOrgsResponse } from "@/lib/discoveryResponseShape";
...
const ENDPOINT = "/api/me/discovery/orgs";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const orgs = await getDiscoveryOrgs(serviceSupabase, {
      limit: validated.limit,
      offset: validated.offset,
      q: validated.q,
    });
    const shaped = shapeDiscoveryOrgsResponse(orgs);
    logDiscoveryAccess(serviceSupabase, {
      user_id: user.id,
      endpoint: ENDPOINT,

/**
 * POST: Reorder within a relation_type. Body: { relationType: string, orderedIds: string[] }
 */
...
const RELATION_TYPES = ["ambassador", "affiliate", "ecosystem", "subsidiary"];
...
export async function POST(request: NextRequest) {
  ...
  let body: { relationType?: string; orderedIds?: string[] };
  try {
  ...
  const relationType = typeof body?.relationType === "string" ? body.relationType.trim().toLowerCase() : "";
  if (!RELATION_TYPES.includes(relationType)) {
    return fail("BAD_REQUEST", "relationType must be ambassador, affiliate, ecosystem, or subsidiary", 400);
  }
  ...
  if (allowedIds.size !== orderedIds.length) {
    return fail("FORBIDDEN", "Some ids are not yours or not found", 403);
  }

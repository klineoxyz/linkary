-- Expose ethos_score on public profile views (gated by analytics_visibility for public view).
-- Enables public one-pager header to show XScore | ETHOS | Index pills.
...
-- Preview view (owner-only, service role): same gating for ethos_score.
-- DROP then CREATE so we can add ethos_score column (REPLACE would require same column list).

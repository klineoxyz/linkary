  ...
  return (
    <div className="space-y-6">
      <header className="crm-page-header">
        <h1 className="crm-page-title">Users & plans</h1>
        <p className="crm-page-subtitle">
          Profile subscription fields only (no org uplift). Read-only. PII: handle ops access carefully.
        </p>
      </header>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.profile_id} className="border-b border-[var(--crm-border)]/60 align-top">
                <td className="py-2 pr-3 font-mono text-[10px] break-all max-w-[120px]">{r.profile_id}</td>
                ...
                <td className="py-2 font-mono text-xs">{r.subscription_status ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>

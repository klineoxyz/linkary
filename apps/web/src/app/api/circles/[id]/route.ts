/**
 * GET /api/circles/[id] — get circle and members
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: circle, error: circleError } = await supabase
    .from("circles")
    .select("id, name, description, visibility, status, owner_type, owner_id, created_at, updated_at")
    .eq("id", id)
    .single();
  if (circleError || !circle) return NextResponse.json({ error: "Not found" }, { status: 404 });
  ...
  const membersWithProfile = (members ?? []).map((m: { id: string; profile_id: string; notes: string | null; created_at: string }) => ({
    id: m.id,
  ...
  return NextResponse.json({
    circle: { ...circle, members_count: membersWithProfile.length },
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.visibility === "shareable" || body.visibility === "invite-only" || body.visibility === "private") updates.visibility = body.visibility;
  if (body.status === "active" || body.status === "archived" || body.status === "draft") updates.status = body.status;

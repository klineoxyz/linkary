...
- **Public DTO:** Only published entities expose partner rows. The DTO returns two arrays, `affiliates[]` and `ambassadors[]`, with allowlist fields: `name`, `website_url`, `logo_url`, `description`, `since_date` (no internal ids).
- **URLs:** All URL fields are sanitized on write (API and DTO mapping); only `http`/`https` are allowed.
- **Editor:** Profile edit page has a “Partner programs” section with tabs (Affiliates | Ambassadors), list, add/edit modal, delete, and reorder (Up/Down via `sort_order`).
...
- **Public DTO:** Only published entities; allowlist: `id`, `title`, `description`, `proof_url`, `created_at`. `proof_url` is sanitized.
...
- Editor shows Copy, Open, and note: “Public updates can take up to 1 minute to appear.”
...
- **Public API:** Allowlist-only DTO; no internal ids or private fields in response.

...
5. **Duplicate creator slug or other DB constraint**  
   - `crm_workspaces` has a UNIQUE constraint on `slug`. If `getOrCreateCreatorWorkspaceAndBoard` tries to insert and slug already exists (e.g. race), insert fails (e.g. 23505). Server now returns `workspace_insert` and logs the error code.
...
| **Missing profile** — authenticated user has no `profiles` row | User sees an actionable message (e.g. “Your account isn’t set up for Tasks yet” with hint to sign in on linkary.xyz first). Server logs `reason=no_profile`. |

      ...
      <div className="max-w-[1200px] mx-auto px-8 py-8 space-y-6">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between mb-6">
            ...
            <span className="inline-flex items-center rounded-full border border-border bg-accent px-3 py-1 text-xs font-medium text-primary">
          ...
          {currentPlan.discount.active && (
            <div className="rounded-lg border border-border bg-accent p-4 mb-6">
              <div className="flex items-start gap-3">
                <AlertCircle className="h-5 w-5 text-primary flex-shrink-0" />
                <div>
                  <div className="text-sm font-medium text-foreground mb-1">Special Pricing Active</div>
                  <div className="text-sm text-muted-foreground">{currentPlan.discount.label}</div>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="h-10 px-4 rounded-lg bg-primary hover:opacity-90 text-white font-medium transition-colors flex items-center gap-2"
            >
              Upgrade Plan
            ...
            <button className="h-10 px-4 rounded-lg border border-border bg-white hover:bg-destructive/10 text-destructive font-medium transition-colors ml-auto">
              Cancel Subscription
            </button>
          </div>
        </div>
                ...
                {paymentHistory.map((payment) => (
                  <tr key={payment.id} className="border-b border-zinc-100">
                    ...
                    <td className="py-3 px-4 text-right">
                      <button className="inline-flex items-center gap-1 text-sm text-primary hover:opacity-90 font-medium">
      ...
      {showAddPayment && (
        <div className="fixed inset-0 bg-zinc-900/20 flex items-center justify-center z-50 p-4">
          <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-xl max-w-md w-full">
            ...
            <div className="flex gap-3">
              <button className="flex-1 h-11 rounded-lg bg-primary hover:opacity-90 text-white font-medium transition-colors">
                Save Card
              </button>
              <button

/**
 * POST /api/deals/[id]/cancel — set gig_deal status to cancelled (owner only).
 */
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if ((deal as { owner_profile_id: string }).owner_profile_id !== ownerProfileId) {
    return fail("FORBIDDEN", "Only the deal owner can cancel it", 403);
  }
  const status = (deal as { status: string }).status;
  if (status !== "active") return fail("BAD_REQUEST", `Deal is ${status}, cannot cancel`, 400);

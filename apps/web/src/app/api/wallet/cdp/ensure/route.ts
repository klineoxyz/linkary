 ...
 * Single canonical wallet provisioning endpoint. Requires existing Supabase session (X login).
 * GET: returns current wallet or { needsCreate: true }. POST: persist client-provided address (client creates via CDP SDK first).
 * Wallet stored in profiles.cdp_wallet_address and cdp_wallets. Display as "Wallet (CDP) 0x...".
 */
...
export async function POST(request: Request) {
  ...
  if (username) {
    const serviceKeyForHandles = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (serviceKeyForHandles) {
      const service = createClient(supabaseUrl, serviceKeyForHandles);

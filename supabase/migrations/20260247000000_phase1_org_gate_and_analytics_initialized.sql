-- =============================================================================
-- Phase 1 Stabilization: org creation gate (company only) + analytics_initialized_at
-- =============================================================================

-- 1) profiles.analytics_initialized_at: set by worker when x_backfill_90d job completes
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS analytics_initialized_at timestamptz;

COMMENT ON COLUMN public.profiles.analytics_initialized_at IS 'Set when 90-day X analytics backfill job completes successfully for this profile.';

-- 2) create_org_and_membership: only company accounts can create orgs (defense in depth)
...
BEGIN
  ...
  SELECT account_type INTO p_account_type FROM public.profiles WHERE id = uid;
  IF p_account_type IS DISTINCT FROM 'company' THEN
    RAISE EXCEPTION 'ORG_COMPANY_REQUIRED: Only company accounts can create an organization.';
  END IF;
...
COMMENT ON FUNCTION public.create_org_and_membership(jsonb) IS 'Creates org and adds caller as owner. Only profiles with account_type=company may call. Slug auto-generated if blank.';
